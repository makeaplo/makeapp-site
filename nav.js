...
(function () {
  // ==== 配置区：按你的站点改这三行 ====
  var SITE_NAME = 'makeapp';
  // 部署在子路径（如 GitHub Pages 的 /makeapp-site/）时自动加前缀；绑自定义域名部署到根路径时自动为空
  var BASE = location.pathname.indexOf('/makeapp-site') === 0 ? '/makeapp-site' : '';
  var LINKS = [
    { href: BASE + '/focuscull/', label: 'FocusCull' },
    { href: BASE + '/#work', label: '工作' },
    { href: BASE + '/#projects', label: '项目' },
    { href: BASE + '/#life', label: '生活' },
    { href: BASE + '/#about', label: '关于' },
  ];
  var THEME_KEY = 'makeapp-site-theme';
  // ==== 配置区结束 ====
  var themable = true;

  /* ---------- 样式 ---------- */
  var css = [
    '.sk-nav{position:fixed;top:0;left:0;right:0;z-index:9990;height:64px;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:0 max(22px, calc((100vw - 860px)/2));',
    'background:rgba(1,1,2,.78);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);',
    'border-bottom:1px solid rgba(35,37,42,.55);',
    ...
    '[data-theme="light"] .sk-nav{background:rgba(255,255,255,.8);border-bottom:1px solid rgba(230,232,238,.7);}',
  ...
  if (themable) {
    ...
    function resolve() { return saved || 'dark'; } // Linear 风格默认深色
    function apply(t) {
